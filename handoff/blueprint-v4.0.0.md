...
version: 4.0.0
status: Draft (awaiting document-reviewer → synth-architecture-auditor → synth-cross-artifact-auditor review)
supersedes:
  - {id: blueprint-feature-pipeline, version: 3.0.0}
superseded_by: []
generated: 2026-05-12
generated_by: synth-designer (user-approved supersession of v3.0.0)
...
v3_to_v4_changes_summary:
  ...
  - Total sub-agents: 27 (was 18 in v3) — added 9 per-layer designers, 1 composer, 1 PRD author, 1 document-reviewer; removed 1 (synth-designer; renamed but functionally subsumed by composer + designers)
---
...
| document-review-knowledge | (lives in document-reviewer's body + skills frontmatter references documentation-criteria) | Loaded by document-reviewer | Gate 0 + Gate 1 discipline; doc_type-specific checks |
...
1. Invokes document-reviewer with `doc_type: IntentClarification` on the approved doc.
2. If verdict is `needs_revision` or `rejected`, invokes synth-reconcile to produce revised version; loops up to 4 cycles per fixed-point discipline.
3. When approved, invokes synth-prd-author with the approved Intent Clarification doc path.
4. Invokes document-reviewer with `doc_type: PRD` on synth-prd-author's output.
5. Loops as above if needs revision.
6. Presents PRD Approval Gate to the user via AskUserQuestion `[approve / refine / cancel]`.
7. If approved, transitions to Stage 2.
8. If `refine`, invokes synth-reconcile with the user's refinement input and produces `01-5-prd-v(N+1).md`.
9. If `cancel`, transitions to CANCELLED terminal state.
...
2. For each flag set to `true`, dispatches the corresponding per-layer designer via Agent tool. Parallel dispatch using multiple Agent invocations (Claude Code platform supports concurrent sub-agent calls).
...
4. Invokes synth-designer-composer with:
   - All per-layer outputs (5a-*-design.md files)
   - All per-layer dependency sidecars (5a-*-dependencies.json files)
   - Approved PRD path
   - Codebase analysis JSON path
   - Rationale brief
5. Composer produces `05-blueprint-v1.md`.
6. Invokes document-reviewer with `doc_type: DesignDoc` and `codebase_analysis` parameter populated from `03-codebase-analysis.json`.
7. Loops via synth-reconcile if needs revision.
8. Presents Blueprint Approval Gate.

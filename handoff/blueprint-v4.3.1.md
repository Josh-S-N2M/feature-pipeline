...
version: 4.3.1
status: Accepted (v4.3.1 produced 2026-05-19 — Path B stage-name surgery applied)
supersedes:
  ...
  - {id: blueprint-feature-pipeline, version: 4.3.0}
superseded_by: []
generated: 2026-05-19
generated_by: finalize-reconciler (Path B stage-name surgery during Phase 2 implementation; user-directed)
v4_3_0_to_v4_3_1_changes_summary:
  - Stage-N references throughout body and filenames removed per the never-reference-pipeline-stages-by-number discipline established during Phase 2 implementation. The Blueprint's content and architectural commitments are otherwise UNCHANGED.
  - Stage-N prose tokens (Stage 0 through Stage 11 including 1.5/5a/5b/8a/8b) replaced with canonical phase names (Preflight, Intent Clarification, PRD Authoring, Discovery Planning, Discovery Research, Synthesis, per-layer Design, Design Composition, Architecture Audit, Plan Authoring, Test Authoring with sub-phases Acceptance Test Authoring and Phase Validator Authoring, Cross-Artifact Audit, Reconciliation, Task Decomposition).
  - Numbered filename prefixes (01-, 01-5-, 02-, 03-, 04-, 05-, 05a-, 05b-, 06-, 07-, 08-, 08a-, 08b-, 09-, 11-) stripped. Canonical filenames are now semantic: intent-clarification.md, prd-v<N>.md, research-plan.md, codebase-analysis.json, codebase-analysis-report.md, synthesis.md (or synthesis/), <layer>-design.md, <layer>-dependencies.json, blueprint-v<N>.md, blueprint-composer-input.json, adrs/, architecture-audit-issues.json, plan-v<N>.md, acceptance-tests.md, phase-validators.md, cross-artifact-audit-issues.json, tasks.json.
  - "Phase N" references retained UNCHANGED where they refer to v4.2.0 Implementation-Plan-internal phases (Phase 2/Phase 4/Phase 6 etc. of the v4 design plan execution) — these are NOT pipeline stages.
  - "L1/L2/L3" verification-level numbering retained — not pipeline stages.
  - Doc-internal phase numbering in Plan artifacts (Phase 0 — Setup, Phase 1..N — Feature delivery, Phase N+1 — Rollout) retained — these are Plan-internal phases, not pipeline stages.
  - No ADRs added or modified. All 21 ADRs inherited from v4.3.0 carry forward.
  - Re-tooling impact: any orchestrator code, glob patterns, or workflow that depended on the numbered filename convention MUST be updated. The semantic naming is now the canonical convention per this version.
v4_2_0_to_v4_3_0_changes_summary:
  ...
  - Discovery phase refactor (ADR-0021). Discovery Planning discovery-plan-author now MUST consult existing KBs + ADRs before producing the research plan; external research is conditional on KB-gap analysis. Discovery Research explicitly modeled as fan-out (1 × discovery-codebase-researcher + N × discovery-external-researcher invoked once per topic).
...
decisions_carried_forward_from_v4_1_0:
  ...
  - PRD Authoring topology — preserved
  - per-layer Design + Design Composition fan-out-fan-in topology — preserved
...
adrs_added_in_v4:
  ...
  - {id: ADR-0012, version: 1.0.0} (PRD generation as PRD Authoring with intake-prd-author)
  ...
  - {id: ADR-0016, version: 1.0.0} (per-layer fan-out + composer fan-in for Design)
...
v3_to_v4_changes_summary:
  ...
  - Adds PRD Authoring with intake-prd-author sub-agent per ADR-0012
  - Adds new human gate: PRD Approval Gate (sixth human gate in the pipeline)
  - Inverts Design to fan-out (9 per-layer designers) + fan-in (design-composer) per ADR-0016
...
> **Notice:** This blueprint fully supersedes v3.0.0. The v3 architecture is preserved at `/mnt/user-data/outputs/feature-pipeline-round-2/blueprint-v3.md`. This v4 incorporates 8 new ADRs (0011 through 0018), retroactive template-migration of ADRs 0001-0010, fan-out-fan-in restructure of Design, PRD generation as new PRD Authoring, shared-document-reviewer integration at five invocation points, critic renames, and EARS-format acceptance criteria.
...
- **Runbook / Operational Spec**: To be authored at `docs/ops/feature-pipeline-runbook.md` as part of Preflight documentation.
...
complexity_rationale: |
  v4 introduces (1) a new pipeline stage (PRD Authoring) requiring orchestrator changes,
  (2) a fan-out-then-fan-in restructure at Design with 10 new sub-agents requiring coordination
...
- [x] Add PRD Authoring with new sub-agent `intake-prd-author`
- [x] Add new human gate: PRD Approval Gate
- [x] Invert Design to fan-out (9 per-layer designers) + fan-in (design-composer)
...
- [x] Specify canonical discovery-codebase-researcher output schema (`codebase-analysis.json`)
...
- [x] Preflight, Intent Clarification, Discovery Planning, Discovery Research, Synthesis, Architecture Audit through Task Decomposition remain functionally unchanged (Architecture Audit and Cross-Artifact Audit rename their critic sub-agents per ADR-0017 but discipline is preserved)
...
- [x] **review-architecture-auditor (sub-agent)** — Enforces: substantive architectural correctness against synthesis claims, blast-radius analysis, brief-honor verification — Config: `.claude/agents/review-architecture-auditor.md` (renamed from synth-critic-1) — Covers: blueprint at Architecture Audit — Status: `adopted`
- [x] **review-cross-artifact-auditor (sub-agent)** — Enforces: cross-artifact consistency (blueprint ↔ plan ↔ tests), cross-model critique, convergence-bounded iteration — Config: `.claude/agents/review-cross-artifact-auditor.md` (renamed from synth-critic-2) — Covers: blueprint + plan + tests at Cross-Artifact Audit — Status: `adopted`
- [x] **test-phase-validator (sub-agent)** — Enforces: per-phase observable outcomes — Config: `.claude/agents/test-phase-validator.md` — Covers: plan at Test Authoring — Status: `adopted`
- [x] **Preflight skill-frontmatter verification** — Enforces: per ADR-0010, no knowledge skill combines `disable-model-invocation: true` with `skills:` preload — Config: orchestrator preflight logic — Covers: all knowledge skills referenced by sub-agents — Status: `adopted`
...
- **FR-1** The pipeline MUST produce a PRD as a versioned artifact at PRD Authoring, authored by `intake-prd-author`, conforming to the canonical PRD template from `KB-documentation-criteria`.
- **FR-2** The pipeline MUST gate PRD content through a new PRD Approval human gate before Research Planning (Discovery Planning) begins.
- **FR-3** Design (Design) MUST decompose into per-layer Design (fan-out: up to 9 per-layer designers conditional on Layer Scope) and Design Composition (fan-in: design-composer integrates per-layer outputs and authors cross-cutting blueprint sections).
...
- **FR-5** Only design-composer authors ADRs at Design; per-layer designers MUST NOT author ADRs.
- **FR-6** shared-document-reviewer MUST run at five invocation points: (1) after Intent Clarification doc, (2) after PRD, (3) after Blueprint composition at Design Composition, (4) after Plan production at Plan Authoring, (5) after each individual ADR write.
...
- **FR-11** discovery-codebase-researcher MUST emit `codebase-analysis.json` conforming to the canonical schema (ADR-0018) in addition to its markdown research report.
...
- **Performance**: Design Composition (composer fan-in) MUST complete within composer's `maxTurns: 60` budget; if exceeded, escalate to Cycle-Cap gate. per-layer designers MUST complete within `maxTurns: 40` each. shared-document-reviewer Gate 0 checks SHOULD complete within ~10 turns; Gate 1 quality assessment within ~25 turns.
...
|------|---------|-------------|
| `.claude/skills/recipe-feature-pipeline/SKILL.md` | Orchestrator skill body | modified — adds PRD Authoring sequencing, fan-out-fan-in coordination at Design, shared-document-reviewer invocations at 5 points |
...
| `working/feature/<slug>/` | Run working directory | new artifacts: `prd-v1.md`, `<layer>-design.md` (per active layer), `blueprint-composer-input.json`, `codebase-analysis.json` |
...
- **PRD Authoring output**: `working/feature/<slug>/prd-v<N>.md` — Applies to: PRD artifacts only — Enforcement: orchestrator-side filename convention
- **per-layer design outputs**: `working/feature/<slug>/<layer>-design.md` — Applies to: per-layer designer outputs — Enforcement: per-layer designer's knowledge skill teaches the filename
- **Discovery Research codebase analysis JSON**: `working/feature/<slug>/codebase-analysis.json` — Applies to: discovery-codebase-researcher's structured output — Enforcement: per ADR-0018 schema specification
...
- [ ] **When** PRD Authoring begins, the system shall invoke `intake-prd-author` with the approved Intent Clarification document as input
- [ ] **When** `intake-prd-author` produces its output, the system shall write `prd-v1.md` to `working/feature/<slug>/` conforming to the canonical PRD template from `KB-documentation-criteria`
...
- [ ] **While** the PRD Approval Gate is pending user response, the system shall NOT begin Discovery Planning (Research Planning)
- [ ] **If** the user selects `refine`, **then** the system shall invoke finalize-reconciler with the user's refinement direction and produce `prd-v(N+1).md`

#### FR-3: Design fan-out-fan-in — Layer: Claude Code / Project Filesystem

- [ ] **When** per-layer Design begins, the system shall determine activated per-layer designers from `feature-scope.json` Layer Scope flags
...
- [ ] **When** all activated per-layer designers complete, the system shall transition to Design Composition and invoke `design-composer` with all per-layer outputs as input
...
- [ ] **While** per-layer Design is active, per-layer designers shall NOT produce ADR files
...
- [ ] **When** Intent Clarification produces an Intent Clarification document, the system shall invoke shared-document-reviewer with `doc_type: IntentClarification`
- [ ] **When** PRD Authoring produces a PRD, the system shall invoke shared-document-reviewer with `doc_type: PRD`
- [ ] **When** Design Composition produces a Blueprint, the system shall invoke shared-document-reviewer with `doc_type: DesignDoc` and `codebase_analysis` populated from `codebase-analysis.json`
- [ ] **When** Plan Authoring produces a Plan, the system shall invoke shared-document-reviewer with `doc_type: Plan`
...
- [ ] **When** Architecture Audit invokes the architectural critique sub-agent, the system shall invoke `review-architecture-auditor` (not `synth-critic-1`)
- [ ] **When** Cross-Artifact Audit invokes the cross-artifact critique sub-agent, the system shall invoke `review-cross-artifact-auditor` (not `synth-critic-2`)
...
- [ ] **When** discovery-codebase-researcher completes its research, the system shall emit `codebase-analysis.json` conforming to the canonical schema (per ADR-0018)
- [ ] **When** shared-document-reviewer is invoked with `doc_type: DesignDoc`, the system shall pass `codebase-analysis.json` content as the `codebase_analysis` parameter
...
- [ ] **When** Preflight runs, the system shall verify all sub-agent definitions exist and all referenced knowledge skills exist with correct frontmatter (per ADR-0010)
...
| **Architecture Audit (renamed from synth-critic-1/2)** | | | |
...
| Claude Code / Project Filesystem | New | `working/feature/<slug>/prd-v<N>.md` | New artifact per FR-1 |
| Claude Code / Project Filesystem | New | `working/feature/<slug>/codebase-analysis.json` | New artifact per FR-11 / ADR-0018 |
| Claude Code / Project Filesystem | New | `working/feature/<slug>/<layer>-design.md` (per active layer) | New per-layer designer output per FR-3 |
...
- **Invocation Method**: Inline-invoke per ADR-0006 — feature-pipeline orchestrator directly invokes synthesis-claim-extractor, synthesis-entity-grapher, synthesis-internal-critic, synthesis-decision-framer, synthesis-substrate-mapper, synthesis-report-author at Synthesis. No skill-to-skill invocation.
- **Integration Target**: GitNexus MCP and codebase-memory-mcp MCP — for blast-radius analysis and codebase structural research.
- **Invocation Method**: Both configured concurrently in `.mcp.json`; Preflight detects availability and routing per ADR-0007 v2.x; sub-agents access via standard MCP tool calls.

### Code Inspection Evidence

| File/Function | Relevance |
|---------------|-----------|
| `/mnt/user-data/outputs/feature-pipeline-round-2/blueprint-v3.md` | v3 of this blueprint (predecessor); 885 lines covering full pipeline topology, sub-agent inventory, knowledge skill placement, fixed-point iteration discipline. v4 inherits Preflight, Intent Clarification, Discovery Planning, Discovery Research, Synthesis, Architecture Audit, Plan Authoring, Test Authoring, Cross-Artifact Audit, Reconciliation, and Task Decomposition from v3 functionally; modifies Design substantially; adds PRD Authoring. |
...
Direct Impact (this meta-blueprint's physical changes):
  claude-code-fs:
    - Orchestrator skill body modified (PRD Authoring sequencing, Design fan-out-fan-in coordination, 5 shared-document-reviewer invocations)
...
| Design single synth-designer | per-layer Design (per-layer designers) + Design Composition (composer) | Yes (topology change) | v4 orchestrator dispatches per-layer-designers based on Layer Scope flags from Preflight; composer fan-in produces single blueprint artifact per existing v3 contract; downstream stages (6 through 11) see the same blueprint-shape input, just produced by a different internal topology |
...
| Intent Clarification → Discovery Planning direct handoff | Intent Clarification → PRD Authoring → Discovery Planning handoff via PRD | Yes (new intermediary stage) | Discovery Planning now consumes the approved PRD instead of the Intent Clarification doc; Intent Clarification's output (Intent Clarification doc) becomes PRD Authoring's input; the PRD Approval Gate sits between PRD Authoring and Discovery Planning |
...
└────────────────────────────────────────────────────────────────┘
  │
  ├─ Preflight: Preflight (inline in orchestrator)
  │    └─ scope detection, MCP health check, ledger init, skill frontmatter verify
  │
  ├─ Intent Clarification: intake-intent-clarifier → Intent Clarification doc
  │    └─ shared-document-reviewer (doc_type: IntentClarification) ─→ Intent Confirmation Gate ←─ USER
  │
  ├─ PRD Authoring: intake-prd-author → PRD                                                   [NEW v4]
  │    └─ shared-document-reviewer (doc_type: PRD) ─→ PRD Approval Gate ←─ USER                 [NEW v4]
  │
  ├─ Discovery Planning: discovery-plan-author → Research Plan
  │    └─ Research Plan Approval Gate ←─ USER
  │
  ├─ Discovery Research: discovery-external-researcher (×N up to 6 in parallel) + discovery-codebase-researcher
  │    └─ discovery-codebase-researcher emits codebase-analysis.json (canonical schema)   [NEW v4]
  │
  ├─ Synthesis: 6 inlined synthesis sub-agents
  ...
  │
  ├─ per-layer Design: Fan-out per-layer designers (up to 9, conditional on Layer Scope)         [NEW v4]
  │    └─ design-claude-code / -frontend / -backend / -api / -query           [NEW v4]
  │       / -database / -cicd / -iac / -codespaces                                       [NEW v4]
  │
  ├─ Design Composition: design-composer (fan-in, cross-cutting authoring, ADRs)           [NEW v4]
  │    └─ shared-document-reviewer (doc_type: DesignDoc) ─→ Blueprint Approval Gate ←─ USER
  │
  ├─ Architecture Audit: review-architecture-auditor                                            [renamed v4]
  │    └─ CoVe + blast-radius + brief-honor verification
  │    └─ ↺ iterate (4-cycle cap) via finalize-reconciler if needed
  │
  ├─ Plan Authoring: plan-author → Plan
  │    └─ shared-document-reviewer (doc_type: Plan)                                             [NEW v4]
  │
  ├─ Test Authoring: test-acceptance-author (reads blueprint only) + test-phase-validator (reads plan)
  │    └─ acceptance tests in EARS format per ADR-0015                                   [NEW v4]
  │
  ├─ Cross-Artifact Audit: review-cross-artifact-auditor                                          [renamed v4]
  │    └─ CMC + diff-mode + convergence-based termination
  │    └─ ↺ iterate via finalize-reconciler if needed
  │
  ├─ Reconciliation: finalize-reconciler (invoked from Architecture Audit and Cross-Artifact Audit when issues need resolution)
  │
  └─ Task Decomposition: finalize-task-decomposer → final task DAG → BUILD APPROVAL GATE ←─ USER
```
...
USER INTENT (free-form text)
    │
    ▼
[Preflight] → feature-scope.json {feature_slug, layers_in_scope, mcp_status, skill_frontmatter_status}
    │
    ▼
[Intent Clarification: intake-intent-clarifier] → intent-clarification.md
    │   ▶ [shared-document-reviewer doc_type:IntentClarification] → verdict
    │   ▶ INTENT CONFIRMATION GATE ← USER
    ▼
[PRD Authoring: intake-prd-author] → prd-v1.md                              ← NEW v4
    │   ▶ [shared-document-reviewer doc_type:PRD] → verdict
    │   ▶ PRD APPROVAL GATE ← USER                                          ← NEW v4
    ▼
[Discovery Planning: discovery-plan-author] → research-plan.md
    │   ▶ RESEARCH PLAN APPROVAL GATE ← USER
    ▼
[Discovery Research: discovery-external-researcher × N(≤6) + discovery-codebase-researcher]
    │   ├→ research-findings-N.md (per researcher)
    │   └→ codebase-analysis.json (canonical schema per ADR-0018)        ← NEW v4
    │   └→ codebase-analysis-report.md (markdown sibling)
    ▼
[Synthesis: 6-stage synthesis (inlined)] → synthesis/report.md + synthesis/adrs/*.md
    │
    ▼
[per-layer Design: Per-layer designers (up to 9 parallel)]                          ← NEW v4
    │   ├→ claude-code-fs-design.md (if scope checked)                  ← NEW v4
    │   ├→ frontend-design.md (if scope checked)                        ← NEW v4
    │   ├→ ... (other layers as scoped)                                     ← NEW v4
    │   └→ Each emits dependencies_on_other_layers field                    ← NEW v4
    ▼
[Design Composition: design-composer]                                         ← NEW v4
    ...
    │   └ produces blueprint-v1.md (canonical template per ADR-0013)
...
[Architecture Audit: review-architecture-auditor]                                       ← renamed v4
    ...
    │   └ produces architecture-audit-issues.json
    │   ↺ if needs_revision → finalize-reconciler → blueprint-v(N+1).md → re-review (4-cycle cap)
    ▼
[Plan Authoring: plan-author] → plan.md (or plan.json depending on serialization)
    │   ▶ [shared-document-reviewer doc_type:Plan] → verdict
    ▼
[Test Authoring: test-acceptance-author + test-phase-validator (concurrent)]
    │   ├→ acceptance-tests.md (EARS-format scenarios per ADR-0015)     ← NEW syntax v4
    │   └→ phase-validators inline in plan.json + e2e-test.md
    ▼
[Cross-Artifact Audit: review-cross-artifact-auditor]                                     ← renamed v4
    │   ├ CMC (model: opus override) + diff-mode + convergence
    │   └ produces cross-artifact-audit-issues.json
    │   ↺ if needs_revision → finalize-reconciler → re-review (4-cycle cap)
    ▼
[Reconciliation: finalize-reconciler] (invoked from 6 and 9 as needed; produces new versions
    of blueprint / plan / tests carrying forward prior decisions per ADR-0005)
    │
    ▼
[Task Decomposition: finalize-task-decomposer] → tasks.json (final task DAG)
...
|-------------------|----------|--------------------|--------------------|-------------------|---------------------|
| Intent Clarification → Discovery Planning handoff | Orchestrator | Direct (clarified intent → research plan) | Via PRD Authoring (clarified intent → PRD → research plan) | Pipeline topology change in orchestrator skill body | Trace through v4 run: orchestrator MUST invoke intake-prd-author after intake-intent-clarifier completes |
| Design authoring | Orchestrator | Single synth-designer | per-layer Design fan-out + Design Composition | Orchestrator skill body change; dispatches based on `feature-scope.json` Layer Scope flags | Trace: per-layer Design invocations match scoped layers; Design Composition receives all per-layer Design outputs |
...
| Codebase analysis output | discovery-codebase-researcher | Markdown research report only | Markdown report + canonical JSON schema (`codebase-analysis.json`) | KB-codebase-research skill extended with schema specification | shared-document-reviewer at Design Composition receives JSON; blueprint composer reads JSON to populate Fact Disposition Table |

### Main Components

#### Component 1: Orchestrator skill (feature-pipeline)

- **Responsibility**: Top-level coordination of all 12 stages; constructs rationale briefs per ADR-0009; manages the per-feature issues-ledger per ADR-0008; maintains traceability.json per ADR-0005; enforces human gates (6 total); dispatches sub-agents via the Agent tool; coordinates fan-out at per-layer Design and fan-in at Design Composition; invokes shared-document-reviewer at 5 points.
- **Interface**: `/feature-pipeline` slash command; takes user intent as input ($1 / $ARGUMENTS); produces final task DAG at `working/feature/<slug>/tasks.json`. Internal interface: file-path-based handoff to all sub-agents per ADR-0006 inline-invoke pattern.
...
- **Responsibility**: At per-layer Design, each authors its corresponding per-layer Design section per the canonical Blueprint template. Emits `dependencies_on_other_layers` for any cross-layer assumption. Authors layer-scoped EARS-format acceptance criteria. Does NOT author ADRs (composer-only per FR-5). Does NOT author cross-cutting sections.
- **Interface**: Invoked by orchestrator in parallel based on `feature-scope.json` Layer Scope flags; each writes to its corresponding `<layer>-design.md` and may emit a `<layer>-dependencies.json` sidecar for cross-layer dependency declarations.
...
- **Interface**: Invoked by orchestrator at 5 points (after Intent Clarification doc, PRD, Blueprint composition at Design Composition, Plan production, and each individual ADR write); produces structured JSON verdict with severity-categorized issues.
...
- **Responsibility**: At Architecture Audit, substantively audits the blueprint against synthesis claims; performs blast-radius analysis via GitNexus; verifies brief-honor per ADR-0009 Layer-3 checks (decision contradiction, open-item handling, re-surfaced verified issue); produces issues JSON for triage.
- **Interface**: Invoked by orchestrator after shared-document-reviewer passes the Blueprint at Design Composition; consumes blueprint + rationale brief + synthesis claims + GitNexus output.
...
- **Responsibility**: At Cross-Artifact Audit, cross-artifact consistency check across blueprint + plan + acceptance tests + phase validators; CMC posture (declares `model: opus` for cross-family critique); diff-mode input (does not see full upstream context); convergence-based termination + 4-cycle hard cap.
- **Interface**: Invoked by orchestrator after Architecture Audit, Plan Authoring, and Test Authoring complete; consumes diffs (blueprint v(N) vs v(N-1)) + plan + tests + prior round's critique-2 issues.
...
- **Decision rule for "no external research warranted"**: If KB-gap analysis shows existing KBs already address all open questions in the PRD's open-questions section AND no ADRs flag conflicting prior decisions, the Research Plan SHALL declare `external_research: skipped` with explicit rationale per KB. In this case, Discovery Research invokes only the codebase researcher.

#### Component 9: discovery-codebase-researcher sub-agent (existing from v3; renamed in v4.3)

- **Responsibility**: At Discovery Research, analyzes the existing codebase against the Research Plan's internal-facing topics; produces `codebase-analysis.json` per the canonical schema (ADR-0018) plus a markdown sibling `codebase-analysis-report.md`. Always invoked exactly once per pipeline run (not fan-out).
- **Interface**: Invoked by orchestrator at Discovery Research fan-out start; inputs = approved Research Plan + rationale brief + GitNexus MCP availability flag; outputs = `codebase-analysis.json` + `codebase-analysis-report.md`.
...
- **Responsibility**: At Discovery Research, executes one external research topic from the Research Plan. **Generic-with-N-invocations pattern** (ADR-0021): a single sub-agent template invoked once per external topic in parallel. Each invocation receives one topic + one tool selection (web_search / Context7 / Microsoft Learn / vendor docs / specifications site) and produces one research findings file. Fan-out cardinality bounded by ADR-0006 §invocation-budget (≤6 parallel external researchers per Discovery Research run); if the plan declares more than 6 topics, the orchestrator batches them sequentially in groups of ≤6.
- **Interface**: Invoked by orchestrator in parallel batches; each invocation gets a single research topic from `research-plan.md`'s topics list + tool selection + rationale brief; output = `research-findings-N.md` (one file per researcher invocation, indexed by topic).
...
- **Conditional invocation**: If `research-plan.md` declares `external_research: skipped`, this sub-agent is not invoked. Discovery Research in that case consists only of discovery-codebase-researcher.

### Data Representation Decision (New Structures Introduced)

| Criterion | Assessment | Reason |
|-----------|-----------|--------|
| Semantic Fit | Yes for canonical schema in `codebase-analysis.json` | The schema's `focusAreas`, `evidence`, `dataTransformationPipelines` fields are deliberately aligned with shared-document-reviewer's expected input (uploaded template) — exact semantic match. |
...
| Lifecycle Fit | Yes | The JSON's lifecycle mirrors its consumers: produced by discovery-codebase-researcher at Discovery Research, consumed by composer at Design Composition and shared-document-reviewer at Design Composition's review invocation. |
...
DocumentReviewerInvocation:
  inputs:
    ...
    codebase_analysis: optional JSON (per ADR-0018 schema; populated at Design Composition for DesignDoc review)
  output:
...
# Per-layer designer output contract (per-layer Design)
PerLayerDesignerOutput:
  primary:
    path: working/feature/<slug>/<layer>-design.md
    structure: corresponds to "<Layer> Design" section of canonical Blueprint template
  sidecar:
    path: working/feature/<slug>/<layer>-dependencies.json
    schema:
...
Input:
  ...
  Preconditions:
    - Intent Clarification doc exists at working/feature/<slug>/intent-clarification.md
...
Output:
  Type: Markdown PRD at working/feature/<slug>/prd-v<N>.md
  Guarantees:
...
Invariants:
  ...
  - Feature-slug from Preflight is preserved verbatim in PRD frontmatter
```
...
Input:
  Type:
    - All per-layer designer outputs (*-design.md)
    - All per-layer dependencies sidecar files (*-dependencies.json)
    - Approved PRD (prd-vN.md)
    - Codebase analysis (codebase-analysis.json)
    - Rationale brief
  Preconditions:
    - All activated per-layer designers have completed (no partial per-layer Design output)
    - feature-scope.json was used to determine which designers activated
    - Synthesis ADRs are written and inherited via rationale brief
  Validation: Orchestrator checks per-layer designer completion before invoking composer
Output:
  Type: Single integrated Blueprint at working/feature/<slug>/blueprint-v1.md
  Guarantees:
...
Invariants:
  - Composer-only ADR authorship at Design (per FR-5)
...
|-------|----------|--------|--------|
| `feature_slug` | Preflight → all subsequent stages | preserved | Generated at Preflight; appears in every artifact's frontmatter; canonical across all run artifacts and per ADR-0008 across all runs of the same feature |
| `run_id` | Preflight → all subsequent stages | preserved | Generated at Preflight as `<feature-slug>-<timestamp>`; preserved verbatim in all artifact frontmatter and rationale briefs |
| `focusAreas[].fact_id` | Discovery Research (discovery-codebase-researcher) → Design Composition (composer Fact Disposition Table) → Design Composition (shared-document-reviewer Gate 0 check) | preserved | Carried verbatim through; required for cross-artifact reference per ADR-0018 |
| `focusAreas[].evidence` | Discovery Research → Design Composition → Blueprint Fact Disposition Table Evidence column | preserved | Verbatim — code snippet or signature; shared-document-reviewer Gate 1 validates the column carries through |
...
| `dependencies_on_other_layers[].assumption` | per-layer designer → Design Composition reconciliation | transformed | Per-layer designer emits assumption as a tentative claim; composer either confirms (other layer's design supports it) or contradicts (other layer's design conflicts) — recorded in composer's reconciliation rationale |
...
System Invariants:
  ...
  - Per-layer designer states at per-layer Design (PENDING → IN_PROGRESS → COMPLETE) MAY be concurrent across designers; all must reach COMPLETE before transition to STAGE_5B_COMPOSER
  - Each iteration loop at Architecture Audit or Cross-Artifact Audit MAY repeat up to 4 times (cap from blueprint v3 §3.7); a 5th iteration MUST escalate to CYCLE_CAP_ESCALATION
...
`design-frontend` is invoked at per-layer Design when `feature-scope.json` has `frontend: true`. Its sole responsibility is to author the Frontend Design section of the user-feature blueprint under composition. It does NOT:
...
- Approved PRD (`prd-vN.md`) — for Frontend-relevant FRs and NFRs
- Codebase analysis JSON (`codebase-analysis.json`) — `focusAreas` filtered to frontend code paths, `dataTransformationPipelines` where Frontend is an endpoint
- Synthesis report (Synthesis output) — for synthesis claims grounding Frontend design choices

#### Output structure

`design-frontend` writes `frontend-design.md` conforming to a Frontend per-layer section structure that includes:
...
The sidecar `frontend-dependencies.json` lists cross-layer assumptions (typically: API contract assumptions, Backend session-handling assumptions, CDN/static-asset hosting assumptions).
...
- **End-to-end tests**: full user flows; tools per stack (Playwright, Cypress, etc.) — these are typically covered in the e2e-test.md at Test Authoring, not per-layer
...
Composer at Design Composition consumes `frontend-design.md` plus its dependencies sidecar. Composer's role for Frontend integration:
- Verify Frontend's API consumption assumptions match API designer's contract output (`api-design.md`)
...
`design-backend` is invoked at per-layer Design when `feature-scope.json` has `backend: true`. Sole responsibility: author the Backend Design section of the user-feature blueprint. Does NOT author cross-cutting sections, ADRs, or other layers' sections.
...
`design-backend` writes `backend-design.md` covering:
...
Sidecar `backend-dependencies.json` lists cross-layer assumptions (typically: API contract assumptions, Query interface assumptions, IaC service-discovery assumptions, Database schema availability).
...
Composer at Design Composition:
...
`design-api` is invoked at per-layer Design when `feature-scope.json` has `api: true`. Sole responsibility: author the API Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-api` writes `api-design.md` covering:
...
Sidecar `api-dependencies.json` lists cross-layer assumptions (typically: Backend implementation assumptions, IaC routing/gateway assumptions, Frontend consumption assumptions).
...
`design-query` is invoked at per-layer Design when `feature-scope.json` has `query: true`. Sole responsibility: author the Query & Data Access Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-query` writes `query-design.md` covering:
...
Sidecar `query-dependencies.json` lists cross-layer assumptions (typically: Database schema assumptions, Backend transaction-boundary assumptions, caching infrastructure assumptions).
...
`design-database` is invoked at per-layer Design when `feature-scope.json` has `database: true`. Sole responsibility: author the Database Schema & Migration Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-database` writes `database-design.md` covering:
...
Sidecar `database-dependencies.json` lists cross-layer assumptions (typically: IaC database-engine-version assumptions, Query layer compatibility, CI/CD migration-execution timing).
...
`design-cicd` is invoked at per-layer Design when `feature-scope.json` has `cicd: true`. Sole responsibility: author the CI/CD Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-cicd` writes `cicd-design.md` covering:
...
Sidecar `cicd-dependencies.json` lists cross-layer assumptions (typically: Backend build-command assumptions, Frontend build-command assumptions, IaC deployment-mechanism assumptions, Database migration-execution timing).
...
`design-iac` is invoked at per-layer Design when `feature-scope.json` has `iac: true`. Sole responsibility: author the Infrastructure as Code Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-iac` writes `iac-design.md` covering:
...
Sidecar `iac-dependencies.json` lists cross-layer assumptions (typically: Backend deployment-target assumptions, Database engine-version availability, CI/CD deployment-mechanism integration).
...
`design-codespaces` is invoked at per-layer Design when `feature-scope.json` has `codespaces: true`. Sole responsibility: author the Dev Environment Design section. Does NOT author cross-cutting sections, ADRs, or other layers.
...
`design-codespaces` writes `codespaces-design.md` covering:
...
Sidecar `codespaces-dependencies.json` lists cross-layer assumptions (typically: Backend dev-mode startup, Frontend dev-server startup, Database test-instance configuration, CI/CD parity expectations).
...
**PRD Authoring sequencing.** After Intent Clarification (Intent Clarification doc) completes and Intent Confirmation Gate clears, orchestrator:
...
4. When approved, generates fresh rationale brief for intake-prd-author; brief Decisions Inherited section lists all inherited ADRs including ADR-0012 (PRD as PRD Authoring), ADR-0015 (EARS for ACs); Files You Should Read names the approved Intent Clarification doc + PRD template reference in `KB-documentation-criteria`; Open Items lists any user-clarification questions surfaced at Intent Clarification that should inform PRD content.
5. Invokes intake-prd-author with the brief prepended to its invocation prompt; intake-prd-author writes `prd-v1.md`.
...
10. If approved, transitions to Discovery Planning.
11. If `refine`, generates rationale brief incorporating user's refinement direction as a new Open Item; invokes finalize-reconciler to produce `prd-v(N+1).md`.
12. If `cancel`, transitions to CANCELLED terminal state.

The same rationale-brief discipline applies to per-layer Design (per-layer designers) and Design Composition (composer) per ADR-0009 Layer-1 enforcement. Specifically:
- Each per-layer designer at per-layer Design receives a layer-customized brief (Decisions Inherited filtered to layer-relevant ADRs; Files You Should Read scoped to layer-relevant codebase analysis sections; Open Items For Upstream surface any layer-specific concerns surfaced at PRD review).
- design-composer at Design Composition receives a brief enumerating all activated layers and cross-layer dependency declarations from per-layer sidecars; Files You Should Read includes all `*-design.md` outputs and `*-dependencies.json` files.
...
**per-layer Design fan-out + Design Composition fan-in coordination.** After Synthesis (synthesis) completes, orchestrator:
1. Reads `feature-scope.json` Layer Scope flags.
...
4. **Per-layer designer structural check (orchestrator-side, addresses I-AA-001).** For each completed per-layer designer's output:
   - Read `<layer>-design.md` and verify required sections exist per the canonical Blueprint template's `<Layer> Design` subsection structure.
   - Verify EARS-format AC keywords used (When/While/If-then/Where/Ubiquitous) per ADR-0015; no BDD or freeform ACs.
   - Verify `<layer>-dependencies.json` exists (may be empty array if designer made no cross-layer assumptions) and conforms to schema in §Contract Definitions.
   - If structural check fails for a designer's output, invoke finalize-reconciler scoped to that single designer's output; reconcile produces `<layer>-design-v(N+1).md`; re-run structural check; loop with 4-cycle cap; on cap-hit, escalate to Cycle-Cap Escalation Gate with the failing layer named.
   - This is an orchestrator-side inline check, NOT a shared-document-reviewer sub-agent invocation. shared-document-reviewer invocation count remains at 5 per ADR-0017 (shared-document-reviewer reviews the COMPOSED blueprint at Design Composition, not per-layer outputs).
5. **Composer-only ADR authorship enforcement (orchestrator-side scan, addresses I-AA-002).** Before invoking composer, orchestrator scans `working/feature/<slug>/adrs/` for any new ADR files created at per-layer Design (i.e., created after the run's Synthesis ADRs but before composer invocation). If any are found, this is a critical violation of FR-5 (composer-only ADR authorship). Orchestrator:
...
6. Invokes design-composer with:
   - All per-layer outputs (`*-design.md` files) that passed structural check
   - All per-layer dependency sidecars (`*-dependencies.json` files)
   - Approved PRD path
...
7. Composer produces `blueprint-v1.md`.
8. Invokes shared-document-reviewer with `doc_type: DesignDoc` and `codebase_analysis` parameter populated from `codebase-analysis.json`.
...
|---------------|---------|-----------|-------------------|-------------|
| Preflight failure | Skill frontmatter has `disable-model-invocation: true` + `skills:` preload | Preflight verification scan | Halt pipeline with clear error message identifying offending skill | User informed; user fixes skill before re-invocation |
| MCP unavailable | GitNexus configured but not responding | Preflight health check | Fall through to fallback MCP (codebase-memory-mcp); if neither, degraded mode with explicit `meta_warning` | User informed; pipeline proceeds in degraded mode |
...
| Per-layer designer output fails structural check at per-layer Design | Designer's `<layer>-design.md` missing required template subsections, or uses non-EARS AC syntax, or sidecar JSON malformed | Orchestrator-side inline structural check after each per-layer designer completes (addresses I-AA-001) | Invoke finalize-reconciler scoped to that single designer; reconcile produces revised version; re-run structural check; 4-cycle cap; on cap-hit, escalate to Cycle-Cap Gate naming the failing layer | User sees reconcile-iteration for the specific layer; composer invocation deferred until all per-layer outputs pass |
| Per-layer designer attempts to author an ADR | Designer writes an ADR file to `working/feature/<slug>/adrs/` at per-layer Design despite knowledge-skill instruction to surface architectural questions for composer | Orchestrator-side scan post-per-layer-Design (addresses I-AA-002) before composer invocation | Quarantine offending ADR; record `critical` violation in issues-ledger; invoke finalize-reconciler on the offending designer with violation as prior_context; re-run designer; if violation re-occurs, escalate to Cycle-Cap Gate | User informed; designer iteration runs |
...
- **Sensitive data**: The pipeline does NOT log user-provided intent verbatim if it contains apparent PII; intent-clarifier's `intent-clarification.md` is the canonical record; orchestrator-side messages use feature-slug references.
...
5. **Phase 5: Orchestrator skill updates**
   ...
   - Tasks:
     - Add PRD Authoring sequencing logic
     - Add PRD Approval Gate logic
     - Add per-layer Design fan-out + Design Composition fan-in coordination logic
     ...
     - Extend Preflight to verify v4 sub-agent and skill inventory presence
...
7. **Phase 7: Pipeline integration test (vertical slice)**
   ...
   - Tasks:
     ...
     - Verify: PRD Authoring produces PRD; PRD Approval Gate fires; per-layer Design activates appropriate per-layer designers; Design Composition produces integrated blueprint conforming to template; shared-document-reviewer fires at 5 points; renamed critics invoke correctly; EARS-format ACs produced
...
- This is a user-facing decision presented via AskUserQuestion at Preflight when v3 artifacts are detected.
...
- **Input Validation**: User intent at Intent Clarification is free-form text. intake-intent-clarifier processes it; orchestrator does not interpret raw user intent for control flow. AskUserQuestion responses are constrained by the platform (option-selection or text-input as configured). No injection attack surfaces in pipeline control flow.
...
- **Recursion safety**: Sub-agents MUST NOT have `Agent` in their tools list (ADR-0001). Enforced by Claude Code platform; verified by Preflight.
...
- **Intent Clarification → PRD Authoring → Discovery Planning**: Verify Intent Clarification → PRD → Research Plan handoff produces all expected artifacts with cross-references.
- **per-layer Design → Design Composition**: Verify per-layer designers produce expected per-layer outputs, composer produces integrated blueprint with all expected sections.
- **Design Composition shared-document-reviewer**: Verify `codebase_analysis` parameter populated correctly from `codebase-analysis.json`; Fact Disposition Table covers all `focusAreas`.
- **Architecture Audit architecture-auditor**: Verify renamed sub-agent invoked; brief-honor verification fires; blast-radius queries return.
- **Test Authoring acceptance-tester output**: Verify EARS-format AC scenarios produced; mapping to blueprint requirement IDs preserved.
- **Cross-Artifact Audit cross-artifact-auditor**: Verify CMC (model: opus) fires; diff-mode input; convergence-based termination measured.
...
- v4 output: PRD-v1.md (NEW), blueprint-v4.md (canonical template), plan.json or plan-v4.md, acceptance-tests.md (EARS), phase-validators inline, tasks.json, plus 5 shared-document-reviewer verdict JSONs and codebase-analysis.json
...
### Alternative 2: Single-author Design (preserve v3 topology)
...
| 2026-05-12 | 4.1.0 | Reconcile pass after shared-document-reviewer + review-architecture-auditor + review-cross-artifact-auditor review of v4.0.0. Resolves 6 issues: I-CA-001/I-DR-001 (sub-agent inventory arithmetic), I-AA-001 (per-layer designer failure handling at per-layer Design → Design Composition transition), I-AA-002 (composer-only ADR authorship enforcement via orchestrator-side scan), I-DR-002 (Output Comparison self-referential semantics note), I-DR-003 (Skills table KB-review-disciplines entry clarification), I-AA-003 (explicit rationale-brief enumeration for PRD Authoring, per-layer Design, Design Composition). All inherited decisions preserved verbatim. | finalize-reconciler (this session) |
| 2026-05-12 | 4.0.0 | Initial v4 supersession of v3. Adds PRD Authoring; fan-out-fan-in at Design; 5 shared-document-reviewer invocations; critic renames; EARS-format ACs; canonical template adoption; retroactive ADR migration; canonical codebase analysis schema. Eight new ADRs (0011-0018); 11 retroactive ADR migrations. | synth-designer (this session) |

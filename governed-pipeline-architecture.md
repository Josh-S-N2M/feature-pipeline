...
- **A diverse cross-provider panel (quorum) for high-tier pipelines only** — full-tier (feature / execution) gates use a k=3 cross-model panel; low-tier flows keep the single-pass judge. Same-model repetition only cuts variance, not systematic error — and quorum is not a free fix (debate amplifies bias; shared-rubric consensus can be illusory), so the panel is diverse-by-provider and advisory, never a rubber stamp. **Resolution is pinned, not "majority/any-fail" (which is self-contradictory): any member fail → fail; any member abstain → escalate to the human; a real quorum (≥2 of 3 reachable members) is required — a provider outage or a single abstain never default-passes.**
...
Reviewer verdicts are **noisy evals, tracked as such**: observability records each gate's judge *stability* (verdict agreement across re-runs / order-swaps / panel members), not just the verdict — and the human-gate adjudications accumulate as the ground-truth set the judge is calibrated against (the improvement loop). **Each panel member's verdict is its own recorded boundary event** (not only the aggregate `gate.result`), so a crash mid-panel cannot lose a completed member finding on replay — without per-member events a re-run could flip a member from fail to pass (judges are ~28% non-reproducible) and silently drop a real finding (D-DR-1).
...
3. **Propagate down the reverse transitive closure** — one graph walk marks every transitive downstream of a bumped upstream stale; no agent invocations. **The walk seeds from the in-git `derived_from` graph** — the very edges the digests are stamped on (move 1) — *not* a separate `pipelines.yaml` traversal. The two must not diverge: a completeness check asserts every `derived_from` edge maps to a declared manifest lineage edge, so the stale set can't be silently incomplete. (Walking the manifest while stamping on `derived_from` — two graphs never reconciled — would surface an *incomplete* stale set and let a terminal artifact whose handoff already passed ship stale.)
...
1. **The run-event log is the recovery journal** (its role beside audit and the improvement loop — the log's triple role, see "The improvement loop" below). On restart, completed `stage.complete` / `gate.result` boundaries replay from the log rather than re-execute. The journal append is **atomic** (write-temp-then-rename — `fsync` alone gives durability, not atomicity against a torn line) so a crash mid-write can't leave a torn final line that drops a completed boundary or breaks replay. *Caveat:* a journal that doubles as the audit trail carries a confidentiality obligation — entries are plaintext unless encrypted, so keep credentials/PII out (TB10), and the log is bounded (compact/rotate; don't grow it without limit).
2. **Idempotent actor boundaries (R21).** R5 guarantees an actor is *re-runnable*, not that re-running won't *duplicate* a side effect. Any actor whose write touches the outside world carries an **idempotency key derived by the coordinator from durable run-state — `{run}:{stage}:{cycle}`, never minted by the model** (a key living only in an actor's context vanishes on replay). The **`:{cycle}` term is load-bearing**: reconciliation legitimately re-runs a stage, and a *corrected* cycle-2 write must not be suppressed as a duplicate of cycle 1 — otherwise the stale state survives the exact pass built to fix it. The key is minted **per side effect, not per stage** (a stage with two external writes needs two keys, else a crash between them loses the second). And because real receivers — `git push`, file writes, Slack, generic MCP POSTs — **ignore caller-supplied keys**, idempotency cannot rest on the receiver: the coordinator writes a **durable "effect-committed" record *before* the side effect and checks it on replay** (the AWS-Powertools idempotency-table pattern), so a crash after the effect but before the boundary re-fires nothing. Generate keys at the stage boundary (stable across attempts within a cycle).
...
This **reconciles with freshness** (Part IV): replay restores an *interrupted* run without redoing work; freshness re-derivation deliberately *redoes* a *stale* artifact from clean context. Different triggers (a crash vs. a superseded upstream), different actions — they do not conflict. Recovery is exercised by replaying recorded run-logs against current code in CI, and by deliberately crashing at the worst moments (after a side effect succeeds but before its boundary is written; **after the effect-committed record is written but before the side effect fires** — the inverse window, where replay must not *skip* a write that never happened; after an approval but before execution).
...
- **health / availability check** — a startup probe validates the tool is reachable *and initialized* in the agent's actual run context, not merely configured. **Reachability alone is a false PASS for the exact bug this exists to catch**: an out-of-session `tools/list` (the `mcp-ping.sh` seed) returns success even when serena has *no active project*. The probe must therefore run **in-session at SessionStart**, **execute the registered init step** (e.g. `activate_project`), and then **verify a project-scoped call succeeds** (`get_current_config` / `find_symbol`) — only that proves *initialized*, not merely *reachable*. The existing `.devcontainer/lib/mcp-ping.sh` + `mcp-auth-probe.sh` cover reachability; the in-session init-then-verify step is the addition.
...
| R21 | **Idempotent, replayable execution.** The run-event log is the recovery journal (atomic append); on restart, completed boundaries replay rather than re-run. Any actor with an external side effect carries a coordinator-derived **cycle-scoped, per-side-effect** idempotency key (`{run}:{stage}:{cycle}`, never model-minted) backed by a **durable effect-committed record** checked on replay (receivers ignore caller keys); human approvals are durable boundaries. | document |
...
| R25 | **Untrusted content is data, not instructions.** Externally-fetched content (web/exa, MCP tool output, authoritative-source re-checks) may inform a finding or flag staleness but may never alter a gate verdict, mint an idempotency key, or trigger a write without an independent deterministic check. | both |
| R26 | **Validators are themselves gated.** Every gate-library validator carries committed true-positive *and* true-negative fixtures and is a gated CI artifact; a validator with no passing fixtures cannot be wired into the manifest — a false-positive gate is a silent outage. | both |
| R27 | **Destructive operations are human-confirmed or deny-by-default.** No actor or the coordinator runs an irreversible operation (`git reset --hard`, `rm -rf`, force-push, mass overwrite) without explicit human-gate confirmation or a deny-by-default permission baseline. | both |
| R28 | **Knowledge is governed like tools (the R22 analogue).** Each KB skill is registry-declared with assigned agents + load mode; conformance asserts every agent's declared skill set exists and every installed KB skill is assigned to ≥1 agent; KB load + consultation are observable per run so a never-consulted grant can be pruned and a missing-but-needed KB surfaced (D-KN-5). | both |
| R29 | **Every pipeline is a tiered manifest entry on the one substrate.** Feature, execution, cc-critique, issue-capture, and report-only flows are each a `pipelines.yaml` entry with a declared tier, running on the one orchestration substrate (Dynamic Workflows, D-ORCH-1); the sentinel flags any orphan or prose-orchestrated pipeline. | both |
| R30 | **A secret/PII scan gates commit and packaging.** A deterministic secret / PII / sensitive-data scan passes before any artifact is committed or packaged — git history is permanent (TB2). | both |
| R31 | **Tools declare provenance and degradation.** Each tool/MCP registry entry records provenance + a pinned version (add/update gated on the pin) and declares its unavailability behavior (documented fallback or fail-loud) — no unvetted/floating server enters a run, no tool silently degrades output mid-run. | both |
| R32 | **Shared durable state is mutated under a scoped lock.** Concurrent runs writing canonical, the registry, the knowledge index, or memory acquire a scoped lock or isolated working trees. | both |
| R33 | **Breaking changes carry a compatibility window.** A breaking change to an externally-consumed surface (OTLP event schema, committed run-summary, cross-pipeline canonical) ships with a compatibility window or migration step — not a flag-day for consumers outside the commit. | both |
| R34 | **Runs are cost-bounded.** Each run declares a per-tier budget envelope; the coordinator emits `budget.exceeded` and escalates to the human gate rather than spending unbounded. | both |
| R35 | **Human-gate approvals are authenticated and tamper-evident.** An approval boundary records *which authorized human* approved, integrity-protected against forgery/replay/misattribution — the approval that authorizes machine side-effects carries the same integrity guarantee (R21) the side-effects do. | both |
| R36 | **Time-dependent decisions use a trusted clock.** Freshness, supersession, and replay-ordering (R5/R11/R12/R18/R19) read a monotonic/trusted time source; a backdated or skewed timestamp cannot win a recency-based current-view. | both |
| R37 | **The run-event log is integrity-checked, bounded, and backpressure-safe.** The system-of-record log (recovery R21, ROI R24, most detections) is append-verified/checksummed, size-bounded (compact/rotate), and never blocks or silently truncates a run — the detection layer has its own detection. | both |
| R38 | **Promoted changes are revertable.** A change shipped via the improvement loop (R24) or an atomic contract bump (R9) has defined rollback semantics — how to retract a harmful validator or a mis-judged compatibility window — not promote-only. | both |
| R39 | **Concurrent canonical edits reconcile semantically.** Beyond the lock (R32), two pipelines making individually-valid but logically-conflicting edits to the same canonical surface are detected and reconciled, not silently last-write-wins. | both |
| R40 | **Deny-by-default execution surface.** Each agent role declares the Bash/MCP/write operations it may perform; the substrate audits that no actor exceeds its role (the sandboxing baseline). | both |
...
| A25 | **Validator with no failing fixture** | A gate wired in with no true-negative (failing) fixture — its "pass" proves nothing; the faked-coverage / Goodhart gate (co-occurs with A1, A10). |
| A26 | **Toxic capability combination** | A single agent granted a dangerous capability pair (filesystem + network, db + network) with no rationale or audit — the exfiltration / escalation surface. |
| A27 | **Orphan / prose-orchestrated pipeline** | A pipeline defined in prose or absent from `pipelines.yaml` — it drifts, escapes the sentinel, and runs on no governed substrate (the un-migrated pipeline). |
| A28 | **Destructive automated operation** | An actor or the coordinator running an irreversible op (`reset --hard`, `rm -rf`, force-push) with no human confirmation or deny baseline. |
| A29 | **Untrusted content executed as instruction** | Treating fetched / tool-output text as instructions — letting it flip a verdict, mint a key, or trigger a write (prompt-injection). |
...
| D-KN-5 | **Knowledge (KB skills) is governed with the same registry + lifecycle + observability discipline as tools** (the D-TOOL-1 analogue): each KB skill is registry-declared with assigned agents + load mode; conformance asserts declared ↔ installed ↔ assigned; KB load + *consultation* are observable per run via **namespaced run-event attributes** (`proj.skill.declared/loaded/used`) since OTel GenAI doesn't cover which-skill-loaded — so dead grants prune and a missing-but-needed KB surfaces. Closes the asymmetry where tools were governed and knowledge was not (R28). | Adopted |
| D-FM-1 | **Rules + anti-patterns carry a failure-modes catalog** (`.claude/canonical/failure-modes.yaml`): each rule/anti-pattern maps to its failure modes (software guidewords + misuse-cases), each with what-good-looks-like + a concrete detection mechanism. The catalog is the bridge from "what good looks like" to "how we catch the bad," and a coverage scan against a concern taxonomy (arc42 + ISO 25010 + project concerns) keeps the rule set *complete*, not just internally consistent (the `failure-mode-catalog` completeness review). | Adopted |

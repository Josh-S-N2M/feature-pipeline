...
- Sequence: `synth-extractor` → `synth-grapher` → `synth-critic` → `synth-framer` → `synth-substrate` → `synth-synthesizer`. Same order as standalone synthesize skill.

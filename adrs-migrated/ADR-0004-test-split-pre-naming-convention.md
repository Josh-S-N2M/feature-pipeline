...
Both feed Critique-2 (renamed synth-cross-artifact-auditor per ADR-0017). Cross-consistency check: every acceptance scenario must be reachable by at least one phase-validator path; every phase artifact must be exercised by at least one validator.
...
- Sub-agent: `synth-acceptance-tester` — reads blueprint only.
- Sub-agent: `synth-phase-validator` — reads plan only.
- Knowledge skill: `acceptance-testing-knowledge` — teaches EARS-format AC authoring (post-v4).
...
- Stage 9 (Critique-2, renamed synth-cross-artifact-auditor): adds cross-consistency check (every acceptance scenario reachable by ≥1 phase-validator; every phase artifact exercised by ≥1 validator).

**New dependencies introduced:**
- The orchestrator must enforce the input separation — it MUST NOT pass the plan to synth-acceptance-tester even if it's available.

**Architectural constraints added:**
- synth-acceptance-tester MUST NOT receive the plan as input. Enforced by orchestrator-side allowlist.
- synth-phase-validator's input MUST include the plan and MAY include the blueprint (the latter for context only).
...
- Orchestrator-level enforcement: when invoking synth-acceptance-tester, pass ONLY the blueprint path; do not pass plan path even if exists.
...
- ADR-0017: rename of synth-critic-2 to synth-cross-artifact-auditor — runs the cross-consistency check.

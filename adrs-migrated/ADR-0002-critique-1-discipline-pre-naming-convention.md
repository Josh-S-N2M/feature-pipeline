...
notes_post_v4: Critic was renamed to synth-architecture-auditor per ADR-0017; this ADR continues to describe the CoVe discipline that the renamed agent implements.
---
...
Critique-1 uses **single-critic CoVe**: one `synth-critic-1` sub-agent (renamed to `synth-architecture-auditor` per ADR-0017) runs CoVe-style verification on each blueprint decision, augmented by a code-graph MCP for blast-radius queries and a checklist of issue types (correctness, missing decisions, contradictions with synthesis, blast-radius gaps).
...
- Mitigation requires the synth-critic-1 (now synth-architecture-auditor) sub-agent's checklist to be comprehensive; the human triage gate is a real backstop.
...
- The renaming of synth-critic-1 → synth-architecture-auditor (per ADR-0017) does not change this discipline; the renamed agent still implements single-critic CoVe.

## Architecture Impact

**Components that change:**
- Sub-agent: `synth-critic-1` (later renamed `synth-architecture-auditor` per ADR-0017) — implements CoVe discipline.
- Knowledge skill: `critique-1-knowledge` (later renamed `architecture-audit-knowledge`) — teaches the checklist and CoVe procedure.
...
- Checklist content lives in `critique-1-knowledge` (renamed `architecture-audit-knowledge` per ADR-0017).
...
- ADR-0017: rename of synth-critic-1 to synth-architecture-auditor (does not change discipline).

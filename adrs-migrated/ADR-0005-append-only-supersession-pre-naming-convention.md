...
- Frontmatter discipline burden on every sub-agent. Mitigated by `documentation-criteria` skill (per ADR-0011) loaded by every sub-agent enforcing the convention.
...
- New sub-agent: `synth-reconcile` — walks supersession chain when producing reconciled artifacts.
- Knowledge skill: `documentation-criteria` (per ADR-0011, canonical document skill) — teaches the frontmatter convention.
...
- synth-reconcile MUST enumerate carried-forward decisions in each new version's frontmatter.
...
- synth-reconcile invocation: orchestrator passes existing chain + new evidence; agent produces new version with explicit "decisions carried forward" enumeration.
...
- ADR-0011: documentation-criteria canonical document skill — teaches the frontmatter convention.

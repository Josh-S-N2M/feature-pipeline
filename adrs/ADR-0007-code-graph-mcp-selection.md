...
- Code Wiki provides an architectural-documentation surface design-composer can consume without additional infrastructure.
...
- The pipeline must own the risk-tier mapping on top of GitNexus's primitives, rather than consuming a named tier output. This is critique-1-knowledge (post-rename KB-review-disciplines) content that needs to be written and maintained.
...
- critique-1-knowledge (renamed KB-review-disciplines per ADR-0017): routes to GitNexus by default; documents fallback path to codebase-memory-mcp.
- discovery-codebase-researcher (Stage 3): uses GitNexus's Code Wiki + Cypher queries for structural codebase analysis; falls through to codebase-memory-mcp's search_graph / get_architecture when GitNexus thin.
...
Since GitNexus does not natively output hop-based risk tiers but the pipeline depends on them, KB-review-disciplines specifies the mapping:
...
- ADR-0017: rename of critique-1-knowledge to KB-review-disciplines (the consumer of the risk-tier mapping above).
- Claims: C-R2-0019 through C-R2-0022.

## v4.3.0 retroactive naming-convention update

Per ADR-0019, all sub-agent, knowledge skill, and orchestrator skill references in this ADR have been updated to the v4.3.0 naming convention (phase-prefixed sub-agents, KB-prefixed knowledge skills, recipe-prefixed orchestrator, shared-prefixed cross-phase sub-agents). The pre-update version is preserved at `ADR-0007-code-graph-mcp-selection-pre-naming-convention.md`. The decision recorded in this ADR is unchanged; only entity names are updated for cross-document consistency.

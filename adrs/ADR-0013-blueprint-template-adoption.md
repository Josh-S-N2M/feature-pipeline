...
superseded_canonical_archived_to: adrs/superseded/ADR-0013-pre-consolidation-canonical.md
---
...
Adopt the uploaded Blueprint template (`BluePrint.txt`) as the canonical structure for all blueprints produced by Stage 5. The template lives in `KB-documentation-criteria` per ADR-0011. synth-designer (and per ADR-0016, the per-layer fan-out designers + composer) MUST produce blueprints conforming to this template. Sections corresponding to layers not in scope are marked `N/A — out of scope` per the template's own convention.
...
| Why this | The template explicitly handles multi-layer features (the pipeline's primary use case); has built-in `N/A — out of scope` convention for narrow features; includes Fact Disposition Table that integrates with shared-document-reviewer's codebase-analysis input (ADR-0018); includes Verification Strategy section that gives test-acceptance-author and test-phase-validator structured inputs. |
...
- **Fact Disposition Table** integrates directly with shared-document-reviewer's `codebase_analysis.focusAreas` input (per the uploaded reviewer template — every focusArea must have a Fact Disposition row).
- **EARS-format Acceptance Criteria** matches ADR-0015's adoption decision and provides structured input to test-acceptance-author.
- **Verification Strategy** section bridges blueprint to phase-validators (test-phase-validator) with explicit correctness definitions and early verification points.
...
- Cons: structural inconsistency across features; downstream stages (planner, task-decomposer) can't rely on section presence; shared-document-reviewer's Gate 0 structural check has no fixed target.
...
- Cons: classification step needed; templates drift independently; shared-document-reviewer must support multiple template structures.
...
- Pros: structural completeness; supports multi-layer features naturally; built-in convention for narrow features; integrates with shared-document-reviewer, codebase analysis, EARS ACs, and Verification Strategy in one structural package; matches user-provided template (their preference is load-bearing).
...
- Verification Strategy section gives test-acceptance-author and test-phase-validator structured inputs.
...
- `KB-documentation-criteria`: extended with Blueprint template (per ADR-0011).
- `design-knowledge`: rewritten to teach the template structure rather than ad-hoc design discipline.
- synth-designer (per ADR-0016, becomes design-composer): instructed to produce template-conforming output.
- Per-layer designer sub-agents (ADR-0016): each produces the corresponding per-layer Design section per the template.
- `shared-document-reviewer`: Gate 0 structural check operates against the template's required-section list for `doc_type: DesignDoc`.

**New dependencies introduced:**
- Blueprint output structure depends on the canonical template version. Versioning of the template lives in `KB-documentation-criteria` skill changelog.
...
- The Design Summary YAML block (`design_type`, `risk_level`, `complexity_level`, `blast_radius`, `main_constraints`, `biggest_risks`, `unknowns`) is authored by design-composer in Stage 5b, not by per-layer designers.
- The Verification Strategy section is authored by design-composer, drawing on per-layer designers' contributions to test planning.
- Fact Disposition Table is authored by design-composer, populated from `discovery-codebase-researcher`'s `focusAreas` output (per ADR-0018).
...
- ADR-0017: shared-document-reviewer uses template structure for `doc_type: DesignDoc` Gate 0 checks.
- ADR-0018: discovery-codebase-researcher output schema — populates the Fact Disposition Table.
- Claim C-R3-0023: template conformance for markdown documents requires application-layer enforcement (i.e., shared-document-reviewer Gate 0).

## v4.3.0 retroactive naming-convention update

Per ADR-0019, all sub-agent, knowledge skill, and orchestrator skill references in this ADR have been updated to the v4.3.0 naming convention (phase-prefixed sub-agents, KB-prefixed knowledge skills, recipe-prefixed orchestrator, shared-prefixed cross-phase sub-agents). The pre-update version is preserved at `ADR-0013-blueprint-template-adoption-pre-naming-convention.md`. The decision recorded in this ADR is unchanged; only entity names are updated for cross-document consistency.

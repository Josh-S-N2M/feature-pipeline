...
|---|---|---|---|---|---|
| **B-1** | **CI smoke-test that asserts `claude mcp list` shows all configured servers connected** *(ADOPTED 2026-05-25 by `pipeline-quickwins-hardening-r1` — see `Issues/cross-artifact-divergence-detection-gap/proposal.md`)* | `prd-v3.md` line 353; was v1 FR-7 (P3) reclassified to Won't-Have per I-DR-005 | CI/CD layer was out of scope for r1. Now adopted: a GitHub Actions workflow will run `claude mcp list` against `.mcp.json` and fail on any non-connected server. | Adopted by `pipeline-quickwins-hardening-r1`. | Resolved by adoption. |
...
| **H-4** | GitNexus `GITNEXUS_SKIP_OPTIONAL_GRAMMARS=1` env var smoke-test *(ADOPTED 2026-05-25 by `pipeline-quickwins-hardening-r1` — see `Issues/cross-artifact-divergence-detection-gap/proposal.md`)* | cc-design line 521; ADR-0041 | Partially-verified (critic batch 1, medium confidence). Env var name is the load-bearing detail for avoiding C++ toolchain. | Adopted: the new feature wires a pre-install dry-run smoke test verifying the env var still works on the pinned tag. | Resolved by adoption. |

...
supersedes: blueprint-v1.md
predecessor: working/feature/devcontainer-mcp-provisioning-r1/blueprint-v1.md
...
change_summary: >-
  Propagates the ADR-0040 Serena-narrowing resolution (7→8 touched agents,
  29→28 untouched) consistently across all 13 reviewer-flagged locations
  (AC-CC-3, Design Summary, Agreement Checklist, Fact Disposition C-0445,
  Change Impact Map, Interface Change Matrix, Architecture Overview ASCII,
  Component #2, State Invariants, Sub-Agents intro, Implementation Plan
  step 7). Aligns inventory count with the 8-record postStart inventory
  (eight readiness_probe records per AC-X-2, with companion edit to ADR-0037
  Implementation Guidance line 143). Absorbs three recommended-severity
  cc-design findings (I-DR-003 placeholder convention; I-DR-005 pedagogical
  justification tightening; I-DR-006 jsonc-vs-json fence) explicitly into
  Open Items. Consolidates the design-cicd duplicate row in the Sub-Agents
  table. Enumerates AC-NFR-2-a / AC-NFR-2-c / AC-NFR-2-d. Documents the
  ADR-0040 design-codespaces forward-looking dependency on ADR-0033
  stub-filling as a Known Unknown.
adrs_referenced:
...
- [x] **Claude Code / Project Filesystem** — `.mcp.json` (NEW), 8 `.claude/agents/*.md` `tools:` allowlist edits, `KB-mcp-platform/` (NEW), `KB-mcp-design/` (NEW), augmented `auditing-mcp/`, `.claude/runtime/mcp-events.jsonl` (NEW; gitignored)
...
blast_radius:
  runtime: "All 36 sub-agents see new .mcp.json registrations; 8 agents gain new mcp__ tool entries; the 28 untouched agents preserve the C-0445 zero-mcp__ invariant"
...
- [ ] **AC-NFR-2-c (PRD-inherited; enumerated per reviewer I-DR-BP-005)**: When the augmented `auditing-mcp` runs end-to-end at Gate 6, the system shall report zero BLOCKER findings — same bar as AC-CC-5 / AC-FR-11-c; the bar's gating semantics are OI-3.
...
|---------|------------|-------------|-----------|----------|
| C-0445 | Zero `mcp__` usage across all 36 sub-agents | preserve (for 28 of 36) | Least-privilege per cc-design Principle 5; 8 consumer agents total (3 from base consumer-mapping — review-architecture-auditor, design-cicd, discovery-codebase-researcher — overlap with the 5-agent ADR-0040 Serena allowlist; the other 5 are non-overlapping). ADR-0040 narrows Serena to a 5-agent allowlist; union with the consumer-mapping = 8 unique. | grep-verified, codebase-analysis Batch 5 |
...
This feature does NOT create new sub-agents. It modifies 8 of 36 existing agents' `tools:` allowlists only. Per ADR-0040, the 5 Serena consumers are the agents that touch Python audit-script surface; three of those five overlap with the 7-agent base consumer-mapping (review-architecture-auditor, design-cicd, discovery-codebase-researcher), and the remaining two (design-cc, design-codespaces) bring the union to **8 unique touched agents** with **28 untouched** preserving C-0445.
...
- **Known future requirements**:
  ...
  - Filling auditing-codespaces STUB (ADR-0033) — the augmented `auditing-mcp` OP-5 owns lifecycle audit until then; can hand back cleanly. design-codespaces's ADR-0040 Serena entry is forward-looking on this stub-fill (OI-6).
...
- **Predecessor Blueprint**: `working/feature/devcontainer-mcp-provisioning-r1/blueprint-v1.md` (superseded by this v2)
- **Per-layer Designs**:
...
- **Blueprint v1 Review Findings (resolved in this v2)**:

...
predecessor: working/feature/pipeline-cross-artifact-discipline-r1/prd-v1.md
...
The domain-specific personas below (pipeline maintainer / future feature authors / future reviewers) deliberately replace the canonical end-user / API-consumer / SRE buckets because this feature's Layer Scope is Claude Code only — there is no end-user surface, no API consumer surface, and no SRE / runtime-operator surface in scope. Every story's actor is therefore a pipeline role, not a product role.
...
- [ ] **FR-1 (H3) — Design-realization audit dimension on `review-architecture-auditor`** — Stakeholder: future reviewer + pipeline maintainer — Layer: Claude Code
  ...
  - AC-FR-1-c: When `review-architecture-auditor` is invoked AND the OI-A1 resolution exists, then the system shall require the auditor's contract document to name the prescription-extraction mechanism (machine-checkable companion file vs. NLP-style parse of ADR prose) so that downstream authors can produce ADRs the auditor can mechanically inspect.
...
- [ ] **FR-5 (H8) — Live tool-surface drift detection** — Stakeholder: future reviewer + pipeline maintainer — Layer: Claude Code
  ...
  - AC-FR-5-a: When the live `list-tools` response from an MCP server removes a tool that is currently allowlisted to one or more agents, the system shall emit a `BLOCKER`-severity finding identifying the server, the removed tool, and each affected agent's allowlist entry.
  ...
  - AC-FR-5-d: When the live `list-tools` response from an MCP server adds one or more tools relative to the recorded baseline, the system shall emit a `MAJOR`-severity finding identifying the server and the added tools.
  - AC-FR-5-e: When the live `list-tools` response from an MCP server reports a signature change (parameter schema or return shape) on a tool that is currently allowlisted to one or more agents, the system shall emit a `MAJOR`-severity finding identifying the server, the tool, the prior and current signatures, and each affected agent's allowlist entry.
...
- [ ] **FR-11 (§O posture) — Replace "post-ship" time-based deferral triggers with event-triggered or honest-acceptance framings in discipline texts** — Stakeholder: future feature author + pipeline maintainer — Layer: Claude Code
  ...
  **This FR shall NOT retroactively edit** the existing `devcontainer-mcp-provisioning-r1-deferrals/register.md` §O.1 rows (E-3, A-3, D-5, I-1). Per the register's own §O.5, the user already directed "no changes to the feature scope" for that prior feature. R2 establishes the going-forward posture in the discipline texts; it does not edit the existing register entries.
  ...
  - AC-FR-11-c: When this PRD's R2 changes are applied AND any author or tool inspects `devcontainer-mcp-provisioning-r1-deferrals/register.md` §O.1, then the system shall preserve rows E-3, A-3, D-5, and I-1 verbatim (no retroactive edits) and shall record this preservation as deliberate, per the register's §O.5.
...
- **NFR-5 (auditor idempotency — FR-1, FR-5)** — Re-running `review-architecture-auditor` against an unchanged feature run shall produce byte-identical findings JSON. Rationale: cross-artifact comparisons that yield non-deterministic outputs make iteration loops worse, not better.
  ...
  - AC-NFR-5-b: When the runner produces non-identical findings JSON across two identical-input runs, then the system shall emit a `MAJOR`-severity self-diagnostic naming the diverging fields.
...
- **NFR-7 (matrix authoring-time budget — FR-6)** — The roster-matrix authoring discipline shall remain mechanically authorable such that **matrix authoring time per feature stays under 30 minutes wall-clock at a 100-agent inventory** (current inventory ~36; headroom for growth). Rationale: the matrix is one row per agent; the burden is roughly linear. At 100 agents the matrix is ~500 cells; a 30-minute budget gives ~18 seconds per cell which is consistent with the positive-evidence-string discipline (read agent file, write one-line justification). If this budget is exceeded in operational use, the FR-6 cell-granularity (currently positive-evidence-required) is the first dial to relax (per Risks-table mitigation).
...
| FR-6 trigger conditions 3 and 4 require interpretive judgment (does design "indicate" agents will load a new skill? does the skill-coverage decision "name" an agent?) and produce uneven enforcement across runs | Future feature author + future reviewer | Medium | Medium | OI-A6 commissions a mechanical evaluator for trigger conditions 3 and 4 at Design Composition; until resolved, runs may rely on author judgment + reviewer second-pass |
...
- `working/feature/pipeline-cross-artifact-discipline-r1/prd-v1.md` — the predecessor PRD this v2 patches
...
- **v2.0.0 (2026-05-26)** — Patch in response to shared-document-reviewer findings on prd-v1 (I-DR-001 through I-DR-007). Structural changes:
  - **I-DR-001** (recommended): Tightened AC-FR-1-c, AC-FR-8-b, and AC-FR-11-c from ubiquitous form to event-triggered EARS framing. AC-FR-1-c now fires "When `review-architecture-auditor` is invoked AND the OI-A1 resolution exists"; AC-FR-8-b now fires "When `design-cc` consults Principle 9 AND begins authoring the FR-6 roster matrix in the same feature run"; AC-FR-11-c now fires "When this PRD's R2 changes are applied AND any author or tool inspects [the register]". Rationale: the PRD's own thesis is mechanical verification; ubiquitous form weakens the testability standard.
  - **I-DR-002** (important): Rewrote NFR-7 from "operable without structural redesign" (a 'system shall be reliable' anti-pattern) to a time-bounded criterion: matrix authoring time per feature shall remain under 30 minutes wall-clock at a 100-agent inventory. AC-NFR-7-a now measures wall-clock against the matrix file's first-write to last-write timestamps in the run state log. Risks-table mitigation cross-references the new budget.
  - **I-DR-003** (important): Rewrote NFR-9 from "discoverable from existing inventory" (unmeasurable) to a grep-checkable predicate: each new affordance shall be referenced from at least one skill or template named in the consuming agent's frontmatter `skills:` list. AC-NFR-9-a now expresses the grep-verifiable check.
  - **I-DR-004** (recommended): Added OI-A6 — mechanical evaluator for FR-6 trigger conditions 3 and 4 (the interpretive triggers). Owner: Design Composition. Needed by: Design Composition completion. Also added a corresponding Risks-table row naming the uneven-enforcement risk and pointing at OI-A6.
  - **I-DR-005** (recommended): Added AC-NFR-5-b — idempotency action-on-fail. When the runner produces non-identical findings JSON across two identical-input runs, it shall emit a `MAJOR`-severity self-diagnostic naming the diverging fields.
  - **I-DR-006** (recommended): Split AC-FR-5-a (which inlined three severity tiers — BLOCKER on tool removal of allowlisted tool, MAJOR on additions, MAJOR on signature changes) into three ACs, one per severity tier: AC-FR-5-a (BLOCKER on removal of allowlisted tool), AC-FR-5-d (MAJOR on additions), AC-FR-5-e (MAJOR on signature changes on allowlisted tools). AC-FR-5-b and AC-FR-5-c (baseline-absent and unparseable cases) retain their numbering.
  - **I-DR-007** (recommended): Added a one-line preface under `## User Stories` explaining that the domain-specific personas (pipeline maintainer / future feature authors / future reviewers) replace the canonical end-user / API-consumer / SRE buckets because Layer Scope is Claude Code only.
  - Frontmatter: `version: 1.0.0` → `version: 2.0.0`; added `predecessor:` field pointing at prd-v1.md.
  - No changes to: Contingency Split section (tested strong on v1), §O posture self-test framings (passed on v1), FR/NFR numbering for unmodified items, layer scope, stakeholder inventory, glossary.

- **v1.0.0 (2026-05-26)** — Initial PRD draft derived from approved Intent Clarification. 11 FRs across the H-mechanism family (H1, H3, H6, H8, H9) and B-mechanism family (B1, B2, B3, B4, B5) plus the §O posture FR (FR-11). Scope class: FULL. Layer scope: Claude Code only.

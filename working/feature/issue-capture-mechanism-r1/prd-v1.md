...
generated: 2026-05-23T17:30:00Z
...
- **NFR-1: Hook fast-path overhead** — Stakeholder: pipeline orchestrator — Layer: Claude Code
  ...
  - AC-NFR-1-a: When the PreToolUse hook receives a `Task` spawn with `subagent_type != "issue-capture-author"`, the system shall return `permissionDecision: "allow"` with negligible added latency (target: hook script execution under ~100ms wall-clock per invocation on the standard devcontainer; precise threshold finalized at Design when the exact hook runtime is decided).
...
- **NFR-7: Write-path + selected-option observability** — Stakeholder: Josh, future debuggers — Layer: Claude Code
  ...
  - AC-NFR-7-a: When a write occurs in create-mode or update-mode, the system shall record the written path and the user's selected option in an observability destination consistent with project conventions.
...
- [ ] **U-9: Observability log destination** for write-path + selected-option (stderr vs. project-relative file vs. other). Owner: design-claude-code (Stage 5).

...
version: 1.0.0
status: superseded
superseded_by: working/feature/execution-pipeline-design-r1/blueprint-v2.md
superseded_at: 2026-05-22T18:30:00Z
superseded_reason: Architecture Audit cycle 1 conditional_pass; 2 MAJOR + 5 MINOR findings addressed in blueprint-v2.md per reconciliation-log-cycle1.md dispatch
feature_slug: execution-pipeline-design-r1
generated: 2026-05-22T06:00:00Z
generated_by: claude (acting as design-composer, fan-in over per-layer designs)
reviewed_at: 2026-05-22T06:45:00Z
reviewer_verdict: approved (Gate 0 pass with 1 cosmetic note on TOC anchor-rendering for slash-containing section titles; Gate 1 pass — Consistency 94, Completeness 96, Rule compliance 95, Clarity 93)
supersedes: []
adrs_added_in_this_run:
  - ADR-0032 (conventions-canonicalization; pairs D-4 + D-18; subsumes IN-005)
  - ADR-0033 (adr-0029-execution-extension; pairs D-7; closes ADR-0029 forward implications)
  - ADR-0034 (prd-mis-credit-cleanup; no synthesis pairing; end-of-pipeline housekeeping)
derived_from:
...
| ADR-0017 | Document-reviewer integration | **Canonical home for the 4-cycle reconciliation cap** (note: PRD v1.1.0 informally credited ADR-0021 for this; cleaned up in codebase-analysis.md v1.1.1 in-table caption; PRD prose correction pending ADR-0034 OR ADR-0032 housekeeping) |
...
- ADR-0017 / ADR-0021 informal mis-credit in PRD v1.1.0 narrative — cleaned up via ADR-0034 OR ADR-0032 housekeeping (not via PRD supersession)
...
- **ADR-0017** vs **ADR-0021** mis-credit in PRD v1.1.0 narrative — surfaced during Batch E review. ADR-0017 is canonical home for 4-cycle reconciliation cap; PRD prose informally credited ADR-0021. Resolution: ADR-0034 OR ADR-0032 housekeeping.
...
| IN-009 | 5 inherited ADRs (0017, 0021, 0028, 0029, 0030, 0031) | **preserve** all + extend | All 5 inherited. ADR-0017 canonical 4-cycle cap is symmetric-extended to per-task per D-12. ADR-0029 forward-anticipated extension closed via ADR-0033. ADR-0030 mechanism-α pattern is symmetric model for D-15. ADR-0031 canonical-helper-home applied to all 7 new scripts. |
...
**Disposition summary**: 14 INs preserved (with various enhancements/extensions); 5 transformed; 1 N/A (IN-013); 1 out-of-scope for modification (IN-010 prior archive). No INs removed.
...
| `.claude/skills/KB-documentation-criteria/references/templates/` | create (4-5 new templates) | Per D-5 pair pattern + FR-7-a: per-task-execution-result, phase-quality-report, quality-reconciliation-log, state-transitions-log-entry-schema, pipeline-run-summary templates |
...
**Total impact**: 5 new agents + 1 modified agent + 3 new skills + 7 new scripts (+ 1 stub) + 2 file moves + 2 modified skill SKILL.md files + 1 spec file (shared-conventions.md) + 4-5 new templates + 1 settings.json edit + 3 new ADRs = **~28-30 file operations**. All under `.claude/` or `adrs/`; no product-layer files touched.
...
- **Key contracts**: 12-state machine (defined in 5b State Transitions section); dispatch routing per 6-row matrix from `execute-finalize-reconciler`; application-level hook invocations to `log_state_transition.py`.
- **Skills bound**: `auditing-shared` (for `log_state_transition.py`, `validate_pipeline_frontmatter.py` access); `KB-documentation-criteria` (for shared-conventions vocabulary).
...
- **Skills bound**: `ai-development-guide` (per D-11 binding criterion); plus task-domain-specific skills as declared in task spec.
...
- **Skills bound**: `ai-development-guide` (per D-11 binding criterion); `auditing-shared` (for `detect_stubs.py` access).
...
- **Skills bound**: `auditing-shared` (for `run_phase_checks.py`); `KB-review-disciplines` (Gate 0/1 pattern adapted for execution surface).
...
- **Skills bound**: None directly bound; consumes phase-quality-report data structure.
...
#### Contract 4: Dispatch taxonomy (D-14 6-row matrix)
...
| **scope_deviations** (ADR-0033 addition) | scope-deviation-scan | the agent whose deviation was unsurfaced | deviation context + scope-deviation finding |
...
5. **Cascade from execution to upstream is dispatch-routed**: an execution-side error that requires upstream re-authoring (Level 4+) dispatches through the 6-row matrix per D-14; no direct cross-pipeline invocations.
...
7. **Templates** (4-5 new templates per D-5 pair pattern + FR-7-a): can happen in parallel with skill+script work. MUST land before:
...
This Blueprint introduces 5 execution-phase artifact templates (the FR-7-c floor: per-task-execution-result, phase-quality-report, quality-reconciliation-log, frontmatter-validation-report, execution-reconciliation-log) plus 2 beyond floor (state-transitions-log-entry-schema, pipeline-run-summary). Per AC-FR-7-d, additional artifacts produced by future design evolution must conform to AC-FR-7-a + AC-FR-7-b. The template pattern (D-5 pair pattern; doc_type per ADR-0032) is extensible.
...
Future amendments will append new rows here; the prior version is preserved per ADR-0005 (file at `blueprint-v1.md` becomes `status: superseded` if a v1.1.0 or v2.0.0 is authored; new file at e.g. `blueprint-v1-1-0.md` becomes canonical).

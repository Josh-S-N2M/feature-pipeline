...
user_token: PRD-CONFIRM-execution-pipeline-design-r1-20260522T025800Z
generated: 2026-05-22T02:48:00Z
generated_by: claude (acting as intake-prd-author, continuation session)
approved_at: 2026-05-22T02:58:00Z
gate_passed: 2
superseded_by: working/feature/execution-pipeline-design-r1/prd-v1.1.0.md
superseded_at: 2026-05-22T03:05:00Z
superseded_reason: Discovery-driven revision per research-plan.md open questions Q-001, Q-002, Q-003. See prd-v1.1.0.md amendment_log for details.
reviewer_verdict: approved (Gate 0 pass, Gate 1 pass — Consistency 95, Completeness 92, Rule compliance 95, Clarity 92)
---
...
- [ ] **FR-8: Three-way auditing split for GitHub Actions and Codespaces** — Stakeholder: Audit-subsystem maintainers, Future pipeline runs — Layer: Claude Code
  ...
  - AC-FR-8-b: A new skill `auditing-codespaces` shall exist at `.claude/skills/auditing-codespaces/` with its own `SKILL.md` and any audit scripts moved out of `KB-codespaces-platform/scripts/` (or newly authored if none currently exist there).
...
- [ ] **FR-9: `ai-development-guide` skill binding on code-producing sub-agents** — Stakeholder: Project owner, Future pipeline runs — Layer: Claude Code
  ...
  Every execution-phase sub-agent that produces or modifies code shall load the `ai-development-guide` skill.
...
- **Net-new audit scripts.** The pipeline can call audits the project hasn't authored yet; those will surface as findings during real execution and become future feature opportunities.
...
- [ ] **A-1**: The `ai-development-guide` skill exists in the project (uploaded reference confirms) and contains the content the Blueprint will rely on. Validation: read `.claude/skills/ai-development-guide/SKILL.md` directly during the Discovery / Research stage of this feature. Owner: discovery-codebase-researcher. By: before Synthesis.
...
- [ ] **A-3**: The existing `KB-codespaces-platform` either has audit scripts that the new `auditing-codespaces` skill will inherit, or has placeholder for the same. Validation: directory inspection. Owner: discovery-codebase-researcher. By: before Synthesis.

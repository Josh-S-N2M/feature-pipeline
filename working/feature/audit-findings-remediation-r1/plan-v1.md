...
generated: 2026-05-21T19:10:00Z
generated_by: claude (acting as plan-author)
...
- `scripts/scan_memory_secrets.py` — canonical memory-secrets scan
...
4. Modify the existing marker-triage logic: when a marker is found, also check its justification is valid; if not, the marker is treated as absent (underlying finding surfaces at original severity).
5. Update the input-schema parsing to handle the new structured form (`pedagogical_sections: [{ path, justification }]`) instead of the old bare-list form.
6. Update the fence-form parser to expect ` -- justification` after `audit-example`.

Verification: pre/post-fix audit on a fixture (NOT the real repo yet) confirms behavior equivalence + new rejection behavior.
...
### P6.6 — Deliverable packaging (Stage 15 — Stage 13 in v4.5.0+ numbering)
